...
In [this explainer](https://anyakatsevich.github.io/sta240-s26.github.io/explainers/lineplot.html), we saw how to use the `curve` function to plot lines. If you want a plot with many lines, you can just call the `curve` function many times with the `add = TRUE` argument to layer things. In that explainer, we just hard-coded the multiple calls to `curve`. A for loop can clean that up:
